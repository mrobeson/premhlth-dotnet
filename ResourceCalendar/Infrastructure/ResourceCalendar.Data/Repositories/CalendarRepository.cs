using System;
using System.Collections.Generic;
using System.Data.Entity;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace ResourceCalendar.Data.Repositories
{
    public class CalendarRepository : IScheduleRepository, IDisposable
    {
        private readonly SchedulingContext _context;

        public CalendarRepository(SchedulingContext context)
        {
            this._context = context;
        }

        public void Update(Schedule schedule)
        {
            _context.Entry(schedule).State = EntityState.Modified;

            // add and delete appointments
            foreach (var appointment in schedule.Appointments)
            {
                if (appointment.State == TrackingState.Added)
                {
                    _context.Entry(appointment).State = EntityState.Added;
                }
                if (appointment.State == TrackingState.Modified)
                {
                    _context.Entry(appointment).State = EntityState.Modified;
                }
                if (appointment.State == TrackingState.Deleted)
                {
                    _context.Entry(appointment).State = EntityState.Deleted;
                }
            }
            _context.SaveChanges();
        }

        public Schedule GetScheduledAppointmentsForDate(int clinicId, DateTime date)
        {
            var schedule = GetScheduleForClinicAndDate(clinicId, date);

            // populate appointments
            var appointments = _context.Appointments
              .Where(a => a.ScheduleId == schedule.Id
                  && DbFunctions.DiffDays(date, a.TimeRange.Start) == 0)
              .ToList();
            var apptHighlights = GetAppointmentHighlights(date, schedule);

            appointments.ForEach(a => a.Title = apptHighlights
                .Where(h => h.Id == a.Id)
                .Select(h => h.Title)
                .FirstOrDefault()
                );

            schedule.AddExistingAppointments(appointments);
            return schedule;
        }

        private IEnumerable<AppointmentHighlights> GetAppointmentHighlights(DateTime date, Schedule schedule)
        {
            // this code can be moved to the database as a stored procedure or view
            var apptHighlightsTsql = @"SELECT A.Id, '('+AT.Code+') ' + rtrim(P.Name) + ' - ' + rtrim(C.FullName) As Title 
                                   FROM            Patients P INNER JOIN
                                                   Appointments A ON P.Id = A.PatientId INNER JOIN
                                                   Clients C ON P.ClientId = C.Id INNER JOIN
                                                   AppointmentTypes AT ON A.AppointmentTypeId = AT.Id
                                   WHERE        (A.ScheduleId = @p0) AND (DATEDIFF(Day, @p1, A.TimeRange_Start) = 0)";

            return _context.Database.SqlQuery<AppointmentHighlights>(apptHighlightsTsql, schedule.Id, date).ToList();
        }

        private class AppointmentHighlights
        {
            public Guid Id { get; set; }
            public String Title { get; set; }
        }

        private Schedule GetScheduleForClinicAndDate(int clinicId, DateTime date)
        {
            // gets one schedule entity, and then pulls the id from that tracked EF entity
            // Guid scheduleId = _context.Schedules.FirstOrDefault(s => s.ClinicId == clinicId).Id; 

            // gets just the Guid id from the database
            Guid scheduleId = _context.Schedules
                .Where(s => s.ClinicId == clinicId)
                .Select(s => s.Id)
                .FirstOrDefault();
            return new Schedule(scheduleId, DateTimeRange.CreateOneDayRange(date), clinicId);
        }

        private bool disposed = false;

        protected virtual void Dispose(bool disposing)
        {
            if (!this.disposed)
            {
                if (disposing)
                {
                    _context.Dispose();
                }
            }
            this.disposed = true;
        }

        public void Dispose()
        {
            Dispose(true);
            GC.SuppressFinalize(this);
        }
    }
}
